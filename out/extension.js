    ...
    let disposable = vscode.commands.registerCommand('go-struct-auto-fill.fillStruct', async () => {
        ...
        try {
            ...
            await editor.edit(editBuilder => {
                ...
                if (replaceRange) {
                    // 检查是否是同一行的结构体初始化
                    const isSameLineInit = replaceRange.isSameLineInit;
 ...
 * 支持复杂的嵌套结构体值解析
 */
function parseExistingStructFields(document, position, outputChannel, braceRangeOverride) {
...
async function generateOrderedFieldsCode(structName, completionItems, existingFields, outputChannel, matchType) {
    ...
    if (fieldDefinitions.length === 0) {
        ...
        const filteredItems = completionItems.items.filter(item => {
            ...
            if (item.detail && !item.detail.includes(structName))
                return false;
            return true;
        });
 ...
 */
function parseStructFieldsFromDefinition(fieldsText, outputChannel, isSamePackage) {
    const fields = [];
    // 清理文本，移除多余的空白和注释
...
function getGoFilePackageName(fileText) {
    const match = fileText.match(/^\s*package\s+(\w+)/m);
    return match ? match[1] : null;
}
// 新增：递归查找最外层包含光标的结构体声明
function findOutermostStructDeclaration(document, position, outputChannel) {
    // 记录所有大括号对的范围
    const stack = [];
    const structRanges = [];
...
// 新增：根据结构体名查找其大括号范围
function findStructBraceRangeByStructName(document, structName, outputChannel) {
